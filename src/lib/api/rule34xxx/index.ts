...
import { Rule34XXXParsedData, Rule34XXXImage, Rule34XXXRawImageData } from "./rule34xxx.interface";
...
const getRule34XXXImgs = async (query: string): Promise<Rule34XXXImage[]> => {
    ...
    const parsedData: Rule34XXXParsedData = <Rule34XXXParsedData>parser.toJson(data, {
        object: true,
        ...xmlToJsonConfig,
    });
    return parsedData.posts.post
        ? parsedData.posts.post.map(
